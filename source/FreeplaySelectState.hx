...
class FreeplaySelectState extends MusicBeatState{
    public static var freeplayCats:Array<String> = ['tutorial', 'week1'];
    ...
    override function create(){
		...
		BG.color = 0xFF00c2ff;
		add(BG);
        categoryIcon = new FlxSprite().loadGraphic(Paths.image('storymenu/' + freeplayCats[curSelected].toLowerCase()));
    ...
    function changeSelection(change:Int = 0) {
		...
		NameAlpha.screenCenter(X);
		add(NameAlpha);
		categoryIcon.loadGraphic(Paths.image('storymenu/' + (freeplayCats[curSelected].toLowerCase())));
