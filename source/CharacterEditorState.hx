...
class CharacterEditorState extends MusicBeatState
{
	...
	function addCustomisedNotesUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "CustomisedNotes";

				// FlxG.log.add(i);
				arrow0 = new FlxSprite(12, 0);
				arrow0.frames = Paths.getSparrowAtlas(char.notesSkin);
				arrow0.antialiasing = true;
				arrow0.setGraphicSize(Std.int(arrow0.width * 0.3));
				arrow0.animation.addByPrefix('static0', 'purple0');
				arrow0.animation.play('static0', false);

				arrowtext0 = new FlxUIInputText(arrow0.x, arrow0.y + 135, 75, char.notesColor[0], 8);
				
				arrow1 = new FlxSprite(60 + arrow0.x, 0);
				arrow1.frames = Paths.getSparrowAtlas(char.notesSkin);
				arrow1.antialiasing = true;
				arrow1.setGraphicSize(Std.int(arrow1.width * 0.3));
				arrow1.animation.addByPrefix('static1', 'blue0');
				arrow1.animation.play('static1', false);

				arrowtext1 = new FlxUIInputText(arrow1.x, arrow0.y + 135, 75, char.notesColor[1], 8);
				
				arrow2 = new FlxSprite(60 + arrow1.x, 0);
				arrow2.frames = Paths.getSparrowAtlas(char.notesSkin);
				arrow2.antialiasing = true;
				arrow2.setGraphicSize(Std.int(arrow2.width * 0.3));
				arrow2.animation.addByPrefix('static2', 'green0');
				arrow2.animation.play('static2', false);

				arrowtext2 = new FlxUIInputText(arrow2.x, arrow0.y + 135, 75, char.notesColor[2], 8);
				
				arrow3 = new FlxSprite(60 + arrow2.x, 0);
				arrow3.frames = Paths.getSparrowAtlas(char.notesSkin);
				arrow3.antialiasing = true;
				arrow3.setGraphicSize(Std.int(arrow3.width * 0.3));
		...
		tab_group.add(arrow0);
		tab_group.add(arrow1);
		tab_group.add(arrow2);
		tab_group.add(arrow3);
		tab_group.add(arrowtext0);
		tab_group.add(arrowtext1);
	...
	var barColorInputText:FlxUIInputText;
	...
	function addCharacterUI() {
		...
		singDurationStepper = new FlxUINumericStepper(15, healthIconInputText.y + 45, 0.1, char.singDuration, 0, 999, 1);
		...
		noAntialiasingCheckBox.checked = char.elAntialiasing;
		noAntialiasingCheckBox.callback = function() {
			char.antialiasing = false;
			if(!noAntialiasingCheckBox.checked && ClientPref.globalAntialiasing) {
				char.antialiasing = true;
			}
			char.elAntialiasing = noAntialiasingCheckBox.checked;
		};
		...
		positionCameraXStepper = new FlxUINumericStepper(positionXStepper.x, positionXStepper.y + 40, 10, char.camerapos[0], -9000, 9000, 0);
		positionCameraYStepper = new FlxUINumericStepper(positionYStepper.x, positionYStepper.y + 40, 10, char.camerapos[1], -9000, 9000, 0);

		var saveCharacterButton:FlxButton = new FlxButton(reloadImage.x, noAntialiasingCheckBox.y + 40, "Save Character", function() {
			saveCharacter();
		});

		barColorInputText = new FlxUIInputText(noAntialiasingCheckBox.x, saveCharacterButton.y, 75, char.barColor, 8);

		cubo = new FlxSprite(scaleStepper.x, barColorInputText.y).makeGraphic(50, 50, 0xFFFFFFFF, false);

		var reloadCharacterButtom:FlxButton = new FlxButton(barColorInputText.x, barColorInputText.y + 45, "Reload Color", function() {
			cubo.color = FlxColor.fromString(barColorInputText.text);
		});
		...
		tab_group.add(reloadCharacterButtom);
		tab_group.add(cubo);
		tab_group.add(barColorInputText);
		UI_characterbox.addGroup(tab_group);
	}
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == arrowtext1)
			{
				char.notesColor[1] = arrowtext1.text;
			}
			else if(sender == arrowtext2)
			{
				char.notesColor[2] = arrowtext2.text;
			}
			else if(sender == arrowtext3)
			{
				char.notesColor[3]= arrowtext3.text;
			}
			else if(sender == barColorInputText)
			{
				char.barColor = barColorInputText.text;
			}	
			...
			{
				char.camerapos[0] = positionCameraXStepper.value;
				updatePointerPos();
			}
			else if(sender == positionCameraYStepper)
			{
				char.camerapos[1] = positionCameraYStepper.value;
				updatePointerPos();
			}
	...
	function updatePointerPos() {
		...
		if(!char.isPlayer) {
			x += 150 + char.camerapos[0];
		} else {
			x -= 100 + char.camerapos[0];
		}
		y -= 100 - char.camerapos[1];
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			positionCameraXStepper.value = char.camerapos[0];
			positionCameraYStepper.value = char.camerapos[1];

			arrowtext0.text = char.notesColor[0];
	...
	function updatePresence() {
		...
		trace("Character: " + daAnim, "Icon: " + leHealthIcon.getCharacter());
		#end
	}
